...
knitr::opts_chunk$set(warning = FALSE, cache = TRUE, dev = "png", dpi = 120)
...
![](cytometryinr_files/figure-slidy/intensity across markers-1.png){ width=100% }
</div>
<div style="float: right; width: 50%;">
...
```{r clustering with dbscan, cache = FALSE}
...
Here's an example of a MEM Heatmap of the same data. 
For more information about MEM see:

- [Diggins, KE, et al., Nat Methods 2017](https://www.nature.com/articles/nmeth.4149)
- [MEM VUeinnovations](https://mem.vueinnovations.com/)
...
Here's an example of FIt-SNE Running. To install it and run it yourself, follow the instructions on the [FIt-SNE github page](https://github.com/KlugerLab/FIt-SNE)
...
#?basename
source(fitsne_scirpt)
start <- Sys.time()
myfitsne <- fftRtsne(mymatrix, max_iter = 1e3)
Sys.time() - start
